...
# Get the user input and place it in the display field
i=0
...
def find_factorial():
    entire_string = display.get()
    try:
        result = perform_factorial(int(entire_string))
        clear_all()
        display.insert(0,result)
    except Exception:
        clear_all()
...
def perform_factorial(num):
    if num < 0:
        return -1
    if num == 1:
        return 1
    return num * (perform_factorial(num-1))
...
Button(root,text="x!",command=lambda : find_factorial()).grid(row=3,column=5)
